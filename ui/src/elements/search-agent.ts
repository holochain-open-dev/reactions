...
export class SearchAgent extends ScopedElementsMixin(LitElement) {
  ...
  private _knownReactions = new StoreSubscriber(
    this,
    () => this.store?.knownReactions
  );

  private get _filteredAgents(): Array<AgentReaction> {
    let filtered = Object.entries(this._knownReactions.value)
      .filter(([agentPubKey, reaction]) =>
        reaction.nickname.startsWith(this._currentFilter as string)
      )
      .map(([agentPubKey, reaction]) => ({ agentPubKey, reaction }));
  ...
  async searchAgents(nicknamePrefix: string): Promise<void> {
    ...
    await this.store.searchReactions(nicknamePrefix);
  }
  ...
  onUsernameSelected(agent: AgentReaction) {
    ...
    if (agent) {
      ...
      if (this.clearOnSelect) {
        ...
        this._textField.value = agent.reaction.nickname;
      }
      this._overlay.close();
    }
  }
