...
/**
 * @element edit-reaction
 * @fires save-reaction - Fired when the save reaction button is clicked
 */
export class EditReaction extends ScopedElementsMixin(LitElement) {
  /**
   * The reaction to be edited.
   */
  @property({ type: Object })
  reaction: Reaction | undefined;

  /**
   * Label for the save reaction button.
   */
  @property({ type: String, attribute: 'save-reaction-label' })
  saveReactionLabel = 'Save Reaction';
  ...
  firstUpdated() {
    this._avatar = this.reaction?.fields['avatar'];
  ...
  renderField(fieldName: string) {
    return html`
      <mwc-textfield
        id="reaction-field-${fieldName}"
        outlined
        required
        autoValidate
        validationMessage="This field is required"
        .label=${fieldName}
        .value=${this.reaction?.fields[fieldName] || ''}
      ...
      ${
        ...
        <div class="column">
          ...
          <mwc-button
            ...
            .label=${this.saveReactionLabel}
            @click=${() => this.fireSaveReaction()}
          ></mwc-button>
        </div>
      </mwc-card>
    `;
